...
(() => {
  ...
  const validCategory = id => window.JARVIS_V9?.categories?.some(category => category.id === id);
  const defaults = { categories: {}, layout: 'none' };
  const sanitize = value => {
    const categories = {}, source = value?.categories && typeof value.categories === 'object' ? value.categories : {};
    for (const [category, ids] of Object.entries(source)) {
      if (!validCategory(category) || !Array.isArray(ids)) continue;
  ...
  const read = () => { try { return sanitize(JSON.parse(localStorage.getItem(KEY) || '{}')); } catch { return { ...defaults, categories: {} }; } };
  const write = value => { const next = sanitize(value); localStorage.setItem(KEY, JSON.stringify(next)); window.dispatchEvent(new CustomEvent('jarvis:v9-selection-changed', { detail: next })); return next; };
  const toggle = (category, entityId) => {
    if (!validCategory(category) || !validEntityId(entityId)) return read();
    const state = read(), current = new Set(state.categories[category] || []);
