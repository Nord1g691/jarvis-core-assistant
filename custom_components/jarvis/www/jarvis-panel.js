...
(() => {
  ...
  class NativeWebSocket {
    ...
    addEventListener(type, fn) { if (type === "message") this.onmessage = fn; if (type === "open") this.onopen = fn; if (type === "error") this.onerror = fn; if (type === "close") this.onclose = fn; }
    removeEventListener() {}
  }
  ...
  if (nativeFetch) window.fetch = async (input, init = {}) => {
    ...
    let parsed; try { parsed = new URL(url, window.location.href); } catch { return nativeFetch(input, init); }
    if (!parsed.pathname.startsWith("/api/")) return nativeFetch(input, init);
    if (!nativeReady) await new Promise(r => window.addEventListener("jarvis:ha-native-ready", r, { once: true }));
    let body = init?.body; if (body instanceof URLSearchParams) body = body.toString(); if (body instanceof Blob) body = await body.text();
    const id = ++seq; const promise = new Promise((resolve, reject) => pending.set(id, { resolve, reject }));
    send({ type: "request", id, message: { type: "http_request", path: parsed.pathname + parsed.search, method: String(init?.method || input?.method || "GET").toUpperCase(), body: body ?? null } });
    const result = await promise;
    return new Response(typeof result?.body === "string" ? result.body : JSON.stringify(result?.body), { status: result?.status ?? 200, headers: { "Content-Type": "application/json" } });
  };
})();`;
  ...
  class JarvisPanel extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this._frame.addEventListener("load", () => {
        ...
        } catch (err) { console.error("JARVIS bridge injection", err); }
        this._sync();
      });
    }
    ...
    async _handleMessage(event) {
      ...
      try {
        ...
        else if (r.type === "get_panel_data") result = await this._hass.callWS({ type: "jarvis/get_panel_data" });
    ...
    async _httpRequest(r) {
      const p = String(r.path || ""), method = String(r.method || "GET").toUpperCase();
      if (method === "GET" && (p === "/api" || p === "/api/")) return { status: 200, body: { message: "API running." } };
      if (method === "GET" && p === "/api/states") return { status: 200, body: this._states() };
      if (method === "GET" && p.startsWith("/api/states/")) { const id = decodeURIComponent(p.slice(12)), s = this._hass.states?.[id]; return s ? { status: 200, body: s } : { status: 404, body: { message: "Entity not found", entity_id: id } }; }
      if (method === "GET" && p === "/api/services") return { status: 200, body: await this._hass.callWS({ type: "get_services" }) };
      const match = p.match(/^\/api\/services\/([^/]+)\/([^/?]+)/);
      if (match && method === "POST") { let d = {}; try { d = r.body ? JSON.parse(r.body) : {}; } catch {} const sd = d.service_data || d.data || d; const result = await this._hass.callService(decodeURIComponent(match[1]), decodeURIComponent(match[2]), sd); return { status: 200, body: result || {} }; }
      return { status: 404, body: { message: "API Home Assistant non supportée", path: p, method } };
    }
