...
(() => {
  ...
  class JarvisPanel extends HTMLElement {
    ...
    connectedCallback(){
      ...
      this._frame.addEventListener("load",()=>{try{const doc=this._frame.contentDocument;const s=doc.createElement("script");s.textContent=BRIDGE_SOURCE;doc.head.appendChild(s);}catch(err){console.error("JARVIS injection",err)}this._sync();});
    }
