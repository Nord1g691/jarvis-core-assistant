(() => {
  ...
  // SINGLE HUD ENTRY POINT.
  // Home Assistant owns transport only. The V5 iframe owns the visible HUD
  // and the complete V9 runtime/UI. Nothing is bootstrapped in the parent page.
  ...
  class JarvisPanel extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this.style.cssText = [
        ...
        "overflow:hidden", "position:relative", "background:#01050c",
      ].join(";");
      ...
      frame.style.cssText = [
        ...
        "border:0", "display:block", "background:#01050c",
      ].join(";");
      frame.src = HUD_URL;
    ...
    _bootHud() {
      ...
      const inject = (src, id) => new Promise((resolve) => {
        if (doc.getElementById(id)) return resolve(true);
      ...
      // The loader starts its own asynchronous dependency chain. We MUST wait
      // for jarvis:v9-ready before loading the UI bridge. Previously the UI was
      // injected immediately after loader.js's onload, which only meant that
      // loader.js itself had loaded; its 28 dependencies could still be absent.
      const waitForV9 = new Promise((resolve) => {
        const done = (event) => {
          window.removeEventListener("jarvis:v9-ready", done);
          resolve(event?.detail || {});
        };
        window.addEventListener("jarvis:v9-ready", done, { once: true });
      });

      (async () => {
        // Native bridge first: it provides the HA transport before V9 UI code
        // starts evaluating connection/settings state.
        const nativeOK = await inject(
          `/jarvis_static/v9/native-bridge.js?v=${ASSET_VERSION}`,
          "jarvis-native-bridge",
        );

        const loaderOK = await inject(
        ...
        let readyDetail = {};
        if (loaderOK) {
          // loader.js dispatches jarvis:v9-ready after every dependency has
          // been attempted. Add a bounded fallback so a broken dependency
          // cannot leave the HUD waiting forever.
          readyDetail = await Promise.race([
            waitForV9,
            new Promise((resolve) => setTimeout(() => resolve({ timeout: true }), 5000)),
          ]);
        }

        const uiOK = await inject(
        ...
        win.dispatchEvent(new CustomEvent("jarvis:ha-panel-ready", {
          detail: {
    ...
    async _handleMessage(event) {
      ...
      try {
        ...
        switch (request.type) {
          ...
          case "call_service":
            result = await this._hass.callService(
              request.domain, request.service, request.service_data || {},
            );
            break;
          case "http_request":
        ...
        this._reply(message.id, false, null, error?.message ? error.message : String(error));
      }
    }
    ...
    async _httpRequest(request) {
      ...
      if (match && method === "POST") {
        ...
        const result = await this._hass.callService(
          decodeURIComponent(match[1]), decodeURIComponent(match[2]), serviceData,
        );
    ...
    _reply(id, success, result, error) {
      ...
      this._iframe.contentWindow.postMessage(
        { source: "jarvis-ha", type: "result", id, success, result, error }, ORIGIN,
      );
    }
  }
