(()=>{
  ...
  const addCss=(href,key)=>{if(document.querySelector(`link[data-jarvis-${key}]`))return;const x=document.createElement('link');x.rel='stylesheet';x.href=href;x.dataset[`jarvis${key[0].toUpperCase()+key.slice(1)}`]='442';document.head.appendChild(x)};
  const addJs=(src,key)=>{if(document.querySelector(`script[data-jarvis-${key}]`))return;const x=document.createElement('script');x.src=src;x.defer=true;x.dataset[`jarvis${key[0].toUpperCase()+key.slice(1)}`]='442';document.head.appendChild(x)};
  addCss('final-pass.css?v=442','final');
  addCss('final-pulse.css?v=442','pulse');
  addCss('orbit-pro.css?v=442','orbit');
  addCss('orbit-hotfix.css?v=442','orbithotfix');
  addCss('boot-pro.css?v=442','boot');
  addCss('boot-origin.css?v=442','bootorigin');
  addCss('boot-flow.css?v=442','bootflow');
  addCss('boot-finale.css?v=442','bootfinale');
  addJs('final-runtime.js?v=442','final');
  addJs('orbit-pro.js?v=442','orbit');
  addJs('boot-pro.js?v=442','boot');
})();