const DEFAULTS={theme:'classic',blocks:{energy:'under',home:'under',export:'under',security:'under'},activeMenu:'home'};
const clone=o=>JSON.parse(JSON.stringify(o));
let saved=clone(DEFAULTS);try{const x=JSON.parse(localStorage.getItem('jarvis_v41_preview')||'null');if(x)saved={...clone(DEFAULTS),...x,blocks:{...DEFAULTS.blocks,...(x.blocks||{})}}}catch(e){}
...
function applySaved(){document.documentElement.dataset.theme=saved.theme;document.documentElement.style.setProperty('--a',themeMap[saved.theme]);$$('#homeBlocks [data-block]').forEach(el=>{el.style.display=saved.blocks[el.dataset.block]==='under'?'block':'none'});renderMenuOnly()}
